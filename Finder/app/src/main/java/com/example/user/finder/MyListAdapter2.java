...
public class MyListAdapter2 extends BaseAdapter {
    ...
    ArrayList<MyListItem> lostNum;
    ArrayList<MyListItem> lostImage;
    ...
    public MyListAdapter2(Context context, ArrayList<MyListItem> data,  ArrayList<MyListItem> lostNum, ArrayList<MyListItem> lostImage, int layout) {
    ...
    public View getView(int position, View convertView, final ViewGroup parent) {
        ...
        final MyListItem num = lostNum.get(position);
        final MyListItem image = lostImage.get(position);

        holder.deleteButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                AlertDialog.Builder alert = new AlertDialog.Builder((Activity) v.getContext());
                alert.setTitle("내 글 (분실물) 삭제");
...
//                        mHandler.postDelayed(mRunnable, 3000);
                        ...
                        try {
                            ...
                            Log.e("deleteopenStream", lostNum.toString());

                            URL url = new URL(SERVER_ADDRESS + "/lostDelete.php?"
                                    + "lostNum=" + num.text
                                    + "&lostImage=" + image.text);

                            url.openStream();
                            String result = getXmlData("lostDelete_result.xml", "result");
                            Log.e("result", result);
                        } catch (Exception e) {
                            Log.e("delete exception(lost)", "");
                        }
                        ...
                        MyListAdapter2 adapter = new MyListAdapter2(context, data, lostNum, lostImage, R.layout.activity_mypage11_mylist);
                ...
                while (eventType != XmlPullParser.END_DOCUMENT) {
                    if (eventType == XmlPullParser.START_DOCUMENT) {
                        ...
                        if (set == 1) {
                            ...
                            Log.e("lostListItem", content);
                        }
                        else if (set == 3) {
                            lostNum.add(new MyListItem(content));
                            Log.e("lostListNum", content);
                        }
                        else if (set == 4) {
                            lostImage.add(new MyListItem(content));
