package mcore.edu.petDr.implementation;
...
public class SocketDataNetworkManager extends AsyncSocketNetwork  {
	...
	public synchronized void requestData(final String trCode, final DataHandler sendBuf,
		...
		if (finalNetReqOptions.indicator) {
			String indMsg = finalNetReqOptions.indicatorMsg;
			// 화면에서 인디게이터 메시지를 넣었을 경우 새로운 인디게이터 메시지 표시
			if (indMsg.trim().equals("")){
				...
				/*AbstractActivity sender = (AbstractActivity)objSender;
				indMsg = sender.getString(R.string.data_transgering_msg);*/
			}
			progressDialog = ProgressDialog.show((AbstractActivity)objSender, null, indMsg, true, true);
		}
